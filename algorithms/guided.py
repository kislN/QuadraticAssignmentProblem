    ...
    def first_improvement(self, except_fac, dlb=False):
        ...
        for r, row in enumerate(self.adj_matrix):
            ...
            for location in np.where(row == 0)[0]:
                ...
                self.adj_matrix[[s,r]] = self.adj_matrix[[r,s]]
                self.cost_fun = self.cost_function()
                if prev_cost < self.cost_fun:
                    self.adj_matrix[[s,r]] = self.adj_matrix[[r,s]]
                    self.cost_fun = prev_cost

                    return {'adj_matrix': self.adj_matrix, 'delta': 0, 'r': 0, 's': 0}
                elif dlb:
                    bits[r] = 0
        return None

    def run(self, method, dlb=True, eye=True, itertations=100, epoches=10, mu=0.1):
...
print('F is: \n', F)
print('D is: \n', D)
print('\n', test.run_local('first_improvement'))
print(test.cost_fun)